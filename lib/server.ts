...
async function ai(u:Account,instruction:string,input:string,schema?:Record<string,unknown>){
...
async function route(req:Request){
 ...
 if(path[0]==='organize'&&method==='POST'){
  const b=z.object({id:z.string()}).parse(await json(req));const n=await item<Note>(u.id,'note',b.id);const out=await ai(u,'Organize this lecture into useful clear Markdown study notes with headings, key ideas, definitions, examples from source, and a short recall checklist. Do not add facts. Return {"summary":"..."}.',sourcePrompt([n]),summarySchema);const summary=z.string().min(1).max(90000).parse(out.summary);const note={...n,summary};await put(u.id,'note',note);return note;
 }
 if(path[0]==='generate'&&method==='POST'){
  ...
  const out=await ai(u,`Create ${b.count} AP-style multiple-choice practice questions and ${b.count} flashcards based ONLY on the lessons. These are independent practice, not official AP material. Questions should require application, evidence analysis and synthesis at Advanced Placement level where the material supports it. Four plausible options per question, exactly one best answer, a detailed rationale for EACH option explaining why right or wrong. Include an exact verbatim source quote for every question and card. Return {"cards":[{"front":"","back":"","quote":""}],"questions":[{"prompt":"","options":["","","",""],"answer":0,"explanations":["","","",""],"quote":""}]}. answer is zero-based.`,sourcePrompt(notes),studySchema(b.count));
  const parsed=z.object({cards:z.array(z.object({front:text,back:text,quote:text})).min(1).max(30),questions:z.array(z.object({prompt:text,options:z.array(text).length(4),answer:z.number().int().min(0).max(3),explanations:z.array(text).length(4),quote:text})).min(1).max(30)}).safeParse(out);
  if(!parsed.success)fail(502,'The AI returned an incomplete study set. Please try again with fewer questions.');
  const study=parsed.data;
  if(study.cards.some(c=>!evidence(c.quote,notes))||study.questions.some(q=>!evidence(q.quote,notes)))fail(502,'The AI returned a source quote that could not be verified. Please generate again.');
  const deck:Deck={id:crypto.randomUUID(),title:b.title,noteIds:b.noteIds,created:Date.now(),cards:study.cards.map(c=>({...c,id:crypto.randomUUID(),due:Date.now(),interval:0})),questions:study.questions.map(q=>({...q,id:crypto.randomUUID()}))};await put(u.id,'deck',deck);return deck;
 }
