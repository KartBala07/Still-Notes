...
4. For Ollama, start Ollama and select an installed model. **List installed models** queries that computer. The default Ollama choice is `llama3.2:3b`; pairing also auto-selects an installed model when the saved one is missing. Availability and hardware requirements depend on the user's installation.
